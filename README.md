...
Резюме: В данном командном проекте тебе предстоит разработать консольное игровое «rogue-like» приложение на языке программирования Go с использованием библиотеки curses (в версии goncurses для Go) в духе классической игры Rogue 1980 года разработки.
...
- [Chapter III](#chapter-iii)
    - [Реализация игры Rogue Game](#реализация-игры)
        - [Задание 1. Как мы здесь очутились?](#задание-1)
        - [Задание 2. Сущностные сущности игровой игры](#задание-2)
        - [Задание 3. Сгенерированный мир](#задание-3)
        - [Задание 4. Ламповое 2D](#задание-4)
        - [Задание 5. Картридж с батарейкой](#задание-5)
        - [Бонусное задание 6. Ты не пройдешь!](#задание-6)
        - [Бонусное задание 7. Искусство баланса](#задание-4)
        - [Бонусное задание 8. Представь, что ты стол](#задание-8)
        - [Бонусное задание 9. Полное ТРИ-ДЭ](#задание-9)
...
Для начала разработки игры реализуй доменный слой, в котором будут описаны основные игровые сущности. Основные рекомендуемые сущности с базовыми атрибутами (необходимый, но недостаточный перечень) следующие:
- Игровая сессия;
- Уровень;
- Комната;
- Коридор;
...
#### Задание 2.2. Логика противников
...
- 5 видов противников: 
  + Зомби (отображение: зеленый z). Низкая ловкость. Средняя сила и враждебность. Высокое здоровье. 
...
- Удары просчитываются по очереди, в несколько этапов: 
  ...
  + 2 этап — расчет урона. Рассчитывается из силы и модификатора (оружия).
...
Необходимо реализовать слой datalayer, в котором будет производиться сохранение и извлечение данных об игровом прогрессе игрока в файле ``json``. 

После прохождения каждого уровня необходимо сохранять полученную статистику и номер пройденного уровня. 

После перезапуска игры уровни должны генерироваться в соответствии с сохраненной информацией, а прогресс игрока — полностью восстанавливаться (набранные очки, текущие значения характеристик). Также в этом файле должна сохраняться статистика по всем попыткам прохождений.

### Бонусное задание 6. Ты не пройдешь!
- Необходимо генерировать двери между комнатами и коридорами и ключи к ним. Реализуй систему разноцветных ключей наподобие классической DOOM.
...
### Бонусное задание 7. Искусство баланса
...
### Бонусное задание 8. Представь, что ты стол
...
### Бонусное задание 9. Полное ТРИ-ДЭ
- Добавь режим 3D-рендеринга, в котором:
  + Основной вид переключается на 3D-вид от первого лица.
  + 2D-вид остается в режиме миникарты в углу экрана.
  + Управление меняется соответствующим образом: W — вперед, S — назад, A — поворот налево, D — поворот направо.
